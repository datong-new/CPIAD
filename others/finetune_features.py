...
from cascade_helper import Helper as CascadeHelper
from retina_helper import Helper as RetinaHelper
import cv2
...
box_scale = 1.0
momentum = "momentum"
save_image_dir = "../images_p_finetune_features_momentum"

momentum = "no_momentum"
save_image_dir = "../images_p_final"
...
def create_yolo_object_mask(darknet_model, image_path, shape=(500, 500), size=50):
    ...
    for x1, y1, x2, y2 in grids:
        ...
        y_interval = max(16, (y2-y1)//5)
        x_interval = max(16, (x2-x1)//5)
        for i in range(1, 5):
...
def specific_attack(model_helpers, img_path, obj_mask):
    obj_mask = obj_mask.permute(2, 0, 1).unsqueeze(0)
    obj_mask = obj_mask[:, :1, :, :].to(device)
    img = cv2.imread(img_path)
    img = torch.from_numpy(img).float()

    attack_img = cv2.imread(img_path.replace("images", "images_best"))
    attack_img = torch.from_numpy(attack_img).float()
    mask = 1-((attack_img-img)==0).float()
...
#        optimizer.zero_grad()
        ...
        tasks = [
                ...
                #get_attack_loss(model_helpers[2], patch_img),
                #get_attack_loss(model_helpers[3], patch_img),
                ]
        res = loop.run_until_complete(asyncio.gather(*tasks))

       # for i, model_helper in enumerate(model_helpers):
       #     al, on = model_helper.attack_loss(patch_img)
        for i, (al, on) in enumerate(res):
            if isinstance(al, torch.Tensor): 
                al = al.to(device)
            attack_loss += al
            object_nums += on
        """
        feature_loss_0, feature_loss_1 = 0, 0
        for i, feature in enumerate(model_helpers[0].features):
            feature = nn.functional.interpolate(feature.unsqueeze(0), (500, 500))
            feature_loss_0 -= torch.abs(feature * obj_mask).mean()
        for i, feature in enumerate(model_helpers[1].features):
            feature = nn.functional.interpolate(feature, (500, 500))
            feature_loss_1 -= torch.abs(feature * obj_mask).mean()

        lambda0, lambda1 = 10, 20
        attack_loss = attack_loss + lambda0 * feature_loss_0 + lambda1 * feature_loss_1
        """
        ...
        if object_nums==min_object_num and min_loss>attack_loss:
            min_loss = attack_loss
            min_img = patch_img
            if t!=1:
                success_attack=True

        #if t%20==1: print("t: {}, feature_loss0:{}, feature_loss1:{}, attack_loss:{}, object_nums:{}".format(t, feature_loss_0, feature_loss_1, attack_loss, object_nums))
        if t%20==1: print("t: {}, attack_loss:{}, object_nums:{}".format(t, attack_loss, object_nums))
        if object_nums==0:
            success_attack=True
            break
...
#    mask = create_grid_mask()
    ...
    #retina_helper = RetinaHelper()
#    cascade_helper = CascadeHelper()
#    model_helpers = [yolov4_helper, faster_helper, retina_helper, cascade_helper]
    ...
    for i, img_path in enumerate(img_paths):
        ...
        obj_mask = create_faster_object_mask(yolov4_helper.darknet_model, faster_helper.model, img_path)
        #mask = create_strip_mask()
        #mask = create_center_mask()
        success_attack = specific_attack(model_helpers, img_path, obj_mask)
